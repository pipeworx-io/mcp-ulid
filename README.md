...
`tools/list` at `https://gateway.pipeworx.io/ulid/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/generate_ulid \
  -H 'Content-Type: application/json' \
  -d '{"count":1}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/generate_ulid`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
